...
class ProductRepository extends CApplicationComponent
{
    ...
    public function getByFilter(array $mainSearchAttributes, array $typeSearchAttributes)
    {
        ...
        // Производители по своим группам(китай/россия)
        if (!empty($_GET['countryBrand']) and empty($_GET['brand'])) {
            $producersIds = array_column(Yii::app()
                ->db
                ->createCommand()
                ->select('id')
                ->from('{{store_producer}}')
                ->where(['in', 'city', $_GET['countryBrand']])
                ->queryAll(), 'id');

           $criteria->addInCondition('t.producer_id', $producersIds);
        }

        if (isset($_GET['cat'])) {
            foreach ($_GET['cat'] as $attrId => $cat) {

                if (is_array($cat)) {
                    $cat = array_map(function($n){
                        return trim($n);
                    }, $cat);
                    $alias = 'attr_'.$attrId;
                    $ids = array_column(Yii::app()
                        ...
                        ->createCommand()
                        ->select('id')
                        ->from('{{store_attribute_option}}')
                        ->where(['in', 'cat', $cat, ['id' => $attrId]])
                        ->queryAll(), 'id');

                    $criteriaAttribute = new CDbCriteria;
                    $criteria->join .= " JOIN {{store_product_attribute_value}} {$alias} ON t.id = {$alias}.product_id ";
                    $criteria->addInCondition("{$alias}.option_value", $ids);

                    $criteria->mergeWith($criteriaAttribute);
                }
            }
        }
     ...
     * @return CActiveDataProvider
     */
    public function getByFilterAjaxProduct(array $mainSearchAttributes, array $typeSearchAttributes, $sorter = 'position ASC')
    {
        /** @var StoreModule $module */
        $module = Yii::app()->getModule('store');
     ...
     * @return CActiveDataProvider
     */
    public function getByBrandProvider(Producer $producer, array $mainSearchAttributes)
    {
        /** @var StoreModule $module */
        $module = Yii::app()->getModule('store');
