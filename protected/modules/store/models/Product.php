...
class Product extends yupe\models\YModel implements ICommentable
{
    ...
    public function rules()
    {
        return [
            ...
            ['is_new', 'numerical', 'integerOnly' => true],
            ['is_new', 'safe'],
            [
            ...
            ['meta_canonical', 'url'],
            [
                'id, type_id, producer_id, sku, name, slug, price, discount_price, discount, short_description, description, data, is_special, length, height, width, weight, quantity, in_stock, is_new, status, create_time, update_time, meta_title, meta_description, meta_keywords, category_id',
                'safe',
                'on' => 'search',
            ],
        ];
    ...
    public function shortText($str,$num){
      // $str = strip_tags($str);
    ...
    public function getIsProductCart()
    {
     ...
     public function getVariantsGroupName()
    {
        $variantsGroups = [];


        foreach ((array)$this->variants as $variant) {
            $variantsGroups[$variant->attribute->id] = $variant->attribute->title;
        }

        return $variantsGroups;
    }

    public function getVariantsGroupId()
    {
        $variantsGroups = [];


        foreach ((array)$this->variants as $variant) {
            $variantsGroups[$variant->attribute->id] = $variant->attribute->id;
        }
