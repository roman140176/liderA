...
class Attribute extends \yupe\models\YModel
{
    ...
    public function rules()
    {
        return [
            ...
            ['type, group_id, sort, is_filter, is_visible', 'numerical', 'integerOnly' => true],
            ['required, is_visible', 'boolean'],
            ['unit', 'length', 'max' => 30],
            ['rawOptions', 'safe'],
    ...
    // public function getOptionsList($category, $all = false)
    // {
    //     if ($category===null) {
    //         return $this->options;
    //     }
    ...
    //         foreach ($category as $catdb) {
    //             $catIds = array_merge($catIds, array_merge($catdb->getChildsArray(), [$catdb->id]));
    //         }
    //     } else {
    //         $catIds = array_merge($category->getChildsArray(), [$category->id]);
    //     }
    ...
    //     $criteria->with = 'optionValue';
    //     $criteria->addInCondition('optionValue.product_id', $ids);

    //     if ($all === false and isset($_GET['cat'], $_GET['cat'][$this->id])) {
    //         $in = array_map(function($n){
    //             return trim($n);
    //         }, $_GET['cat'][$this->id]);

    //         $criteria->addInCondition('cat', $in);
    //     }
...
public function getOptionsList($category, $all = false)
    {
        ...
        $id = array_column(Yii::app()
            ->db
            ->createCommand()
            ->select('product_id')
            ->from('{{store_product_category}}')
            ->where(['in', 'category_id',$category->id])
            ->queryAll(), 'product_id');
        $id = array_unique($id);
        ...
        $ids = array_column(Yii::app()
            ...
            ->select('id')
            ->from('{{store_product}}')
            ->where(['in', 'category_id', !empty($category->getChildsArray()) ? array_merge($category->getChildsArray(),[$category->id]) : $category->id])
            ->queryAll(), 'id');
    ...
    public function attributeLabels()
    {
        return [
            ...
            'is_visible' => Yii::t('StoreModule.store', 'Основной фильтр'),
        ];
    }
    ...
    public function getTypesList()
    {
        return [
            ...
            self::TYPE_DROPDOWN_PANEL => Yii::t('StoreModule.store', 'Dropdown list Panel'),
        ];
    }
    ...
    {
        return $this->type == self::TYPE_DROPDOWN || $this->type == self::TYPE_CHECKBOX_LIST || $this->type == self::TYPE_DROPDOWN_PANEL;
    }
     public function isDropdownPanel()
    {

        $result = false;

        if($this->type == self::TYPE_DROPDOWN_PANEL){
            $result = true;
        }
