...
<div class='row'>
    <div class="col-sm-8">
        <?= $form->checkBoxGroup($model, 'is_visible'); ?>
    </div>
</div>
...
<?php else: ?>
    <div id="options" class="<?= !$model->isMultipleValues() ? 'hidden' : ''; ?>">
        <div class="row" style="display: flex;align-items: flex-end;">
            <div class="col-md-2">
                <?= CHtml::label('Название', null, ['class' => 'control-label']); ?>
                <?= CHtml::textField('AttributeOption[name]', null, ['class' => 'form-control']); ?>
            </div>
            <div class="col-md-2 <?= !$model->isDropdownPanel() ? 'hidden' : ''; ?>">
                <?= CHtml::label('Тональность', null, ['class' => 'control-label']); ?>
                <?= CHtml::textField('AttributeOption[cat]', null, ['class' => 'form-control']); ?>
            </div>
            <div class="col-md-2">
                ...
                <?php $this->widget(
                    ...
                    [
                        ...
                        'columns' => [
                            ...
                            [
                                'class' => 'bootstrap.widgets.TbEditableColumn',
                                'name' => $model->isDropdownPanel() ? 'cat' : '',
                                'visible' => $model->isDropdownPanel(),
                                'editable' => [
                                    'url' => $this->createUrl('/store/attributeBackend/inlineCat'),
                                    'mode' => 'inlineCat',
                                    'params' => [
                                        Yii::app()->getRequest()->csrfTokenName => Yii::app()->getRequest()->getCsrfToken(),
                                    ],
                                ...
                                'htmlOptions' => [
                                    'class' => $model->isDropdownPanel() ? '' : 'hidden'
                                ]
                            ],
                            [
...
<script type="text/javascript">
    $(function () {
        ...
        $('#add-option-btn').on('click', function (e,i) {
            event.preventDefault();
            var val = $('#AttributeOption_name').val();
            var cat = $('#AttributeOption_cat').val();
            if (val) {
                $.post($(this).data('url'), {
                    'value': val,
                    'cat':cat,
        ...
        $('#attribute-type').change(function () {
            ...
            if ($.inArray(parseInt($(this).val()), [<?= Attribute::TYPE_DROPDOWN;?>, <?= Attribute::TYPE_CHECKBOX_LIST;?>,<?= Attribute::TYPE_DROPDOWN_PANEL;?>]) >= 0) {
