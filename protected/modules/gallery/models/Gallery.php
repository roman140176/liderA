...
class Gallery extends yupe\models\YModel
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            ['self_id', 'numerical', 'integerOnly' => true, 'on' => ['update', 'insert']],
            ['name', 'length', 'max' => 250],
            ['image, pic_name, names, self_id', 'safe'],
            ['status', 'in', 'range' => array_keys($this->getStatusList())],
            ['id, name, description, status, owner, sort,image', 'safe', 'on' => 'search'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'image' => 'Изображение для обложки',
            'names' => 'Сортировка по группе',
            'pic_name' => 'Сортировка по группе и подгруппе',
            'self_id' => 'Родительская галерея',
        ];
    }
     ...
     **/
    public function previewImage($width = 0, $height = 0)
    {
    ...
    public function getGalleryList()
        {
            if (isset($this->id)) {
                return CHtml::listData(self::model()->published()->findAll(['condition'=>'id<>:id', 'params' => [':id' => $this->id]]),'id','name');
            }else{
