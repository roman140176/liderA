...
class YupeModule extends WebModule
{
    ...
    public function getParamsLabels()
    {
        return [
            ...
            'wmode' => 'режим работы',
            'main_phone' => 'основной номер тел.',
            'second_phone' => 'доп.телефон1',
            'thrid_phone' => 'доп.телефон2',
            'reception_phone' => 'приемная(факс)',
    ...
    public function getEditableParamsGroups()
    {
        return [
            ...
            'reqwisits' => [
                'label' =>'Реквизиты Компании / Организации',
                'items' =>[
                    'main_phone',
                    'reception_phone',
                    'second_phone',
    ...
    public function getMessengersWithout(){
        return [
            'whatsapp' => $this->whatsapp,
            'telegram' => $this->telegram,
        ];
    ...
    public function SocialWidget(){
        ...
        foreach ($this->getSocials() as $key => $value) {
            echo "<a href='{$value}' class='soc-link social-item-{$key}' target='_blank'>
            <img src ='{$assets}/images/icons/{$key}.png'/>
            </a>";
            }
    }
    public function MessengersWidget(){
        $assets = Yii::app()->getTheme()->getAssetsUrl();
        foreach ($this->getMessengers() as $key => $value) {
            echo "<a href='{$value}' class='soc-link social-item-{$key}' target='_blank'>
            <img src ='{$assets}/images/icons/{$key}.png'/>
            </a>";
            }
    }
    public function feedbackMessengersWidget(){
        $assets = Yii::app()->getTheme()->getAssetsUrl();
        echo "<span>в</span>";
        foreach ($this->getMessengersWithout() as $key => $value) {
            echo "<a href='{$value}' class='feedback-link feedback-item-{$key}' target='_blank'>
            <img src ='{$assets}/images/icons/{$key}.png'/>
            <span class='m_name'>{$key}</span>
            </a>";
            }
        echo "<span>и</span><a href='{$this->viber}' class='feedback-link feedback-item-viber' target='_blank'>
            <img src ='{$assets}/images/icons/viber.png'/>
            <span class='m_name'>Viber</span>
            </a>";
    }
    public function getPhoneLink($phone){
        $phone =  str_replace([' ','(', ')','-','+'],'',$phone);
        return 'tel:'.str_replace('7','8',$phone);
    }
    public function getSiteLogo($ext){
       $assets = Yii::app()->getTheme()->getAssetsUrl();
       return "<img src ='{$assets}/images/logo.{$ext}' alt='{$this->siteName}'/>";
    }
    /**
